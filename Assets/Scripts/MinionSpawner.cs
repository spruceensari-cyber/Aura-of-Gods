...
public class MinionSpawner : MonoBehaviour
{
    ...
    void StartWave()
    {
        if (blueMinionPrefab == null || redMinionPrefab == null || blueBaseSpawn == null || redBaseSpawn == null)
            return;
    ...
    void SpawnMinion(GameObject prefab, MinionTeam team, Vector3[] path, MinionRole role, float sideOffset, float backOffset, string laneName)
    {
        ...
        m.laneWidth = Mathf.Max(sideSpacing * 1.45f, 1.8f);
        m.formationOffset = sideOffset;
        ...
        else
        {
            ...
            m.attackRate = 1.6f;
            m.speed = 2.4f;
            obj.transform.localScale = new Vector3(originalScale.x * 1.6f, originalScale.y * 1.2f, originalScale.z * 1.6f);
        }
    }
    ...
    Vector3[] BuildBluePath(Transform[] laneWaypoints)
    {
        ...
        if (blueBaseSpawn == null || redBaseSpawn == null) return path.ToArray();
        path.Add(blueBaseSpawn.position);
        if (laneWaypoints != null)
            foreach (Transform t in laneWaypoints)
                if (t != null) path.Add(t.position);
