...
/// Short-lived slash/orb primitives use the shared pool; damage logic remains untouched.
/// </summary>
public class AOGCombatVfxLanguageRuntime : MonoBehaviour
{
    ...
    private static void Install()
    {
        ...
        GameObject host = new GameObject("AOG_Combat_VFX_Language_Runtime");
        DontDestroyOnLoad(host);
        host.AddComponent<AOGCombatVfxLanguageRuntime>();
    }
    ...
    private static void SpawnIdentityImpact(AOGActiveChampion champion,Vector3 point,bool ability,float damage)
    {
        ...
        else if (id.Contains("pyrelle"))
        {
            ...
            for (int i=0;i<4;i++)
                Slash(point,Quaternion.Euler(UnityEngine.Random.Range(-15f,35f),i*90f,UnityEngine.Random.Range(-25f,25f)),new Vector3(0.11f,0.95f*scale,0.11f),new Color(1f,0.42f,0.04f),0.34f);
        }
        else if (id.Contains("selene"))
        {
            Ring(point,1.35f*scale,accent,0.055f,0.45f);
            for (int i=0;i<4;i++)
                Slash(point,Quaternion.Euler(0f,i*45f,45f),new Vector3(0.055f,1.1f*scale,0.055f),Color.Lerp(accent,Color.white,0.45f),0.42f);
        }
    ...
    {
        GameObject ring = AOGAbilityVisuals.CreateRing("Identity_Impact_Ring",point,radius,color,width);
        Object.Destroy(ring,life);
    }

    private static void Slash(Vector3 point,Quaternion rotation,Vector3 scale,Color color,float life)
    {
        AOGTransientPrimitivePoolRuntime.SpawnCube(point,rotation,scale,color,4f,life);
    }

    private static void Orb(Vector3 point,float size,Color color,float life)
    {
        AOGTransientPrimitivePoolRuntime.SpawnSphere(point,Vector3.one*size,color,4.5f,life);
    }
}
